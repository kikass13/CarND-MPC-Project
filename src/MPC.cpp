...
class FG_eval 
{
 public:
	...
	void operator()(ADvector& fg, const ADvector& vars) 
	{
		...
		// cost for divergence/error to optimal state
		...
		// cost of acceleration and actuator 
		for (unsigned int i = 0; i < N - 1; i++) 
		{
			fg[0] += 2 * CppAD::pow(vars[delta_start + i], 2);
			fg[0] += 2 * CppAD::pow(vars[a_start + i], 2);
			// try adding penalty for speed + steer
			fg[0] += 2 * CppAD::pow(vars[delta_start + i] * vars[v_start+i], 2);
		}
		...
		for (unsigned int i = 0; i < N - 2; i++) 
		{
			...
			fg[0] += 3 * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
		}
		...
		// constraints, starts at t=1 because we allredy set initial values for t_0
		for (int t = 1; t < N; t++) 
		{
			// state time t.
			...
			// f(0)
			AD<double> f_t0 = coeffs[0] + coeffs[1] * x_t0 + coeffs[2] * CppAD::pow(x_t0, 2) + coeffs[3] * CppAD::pow(x_t0, 3);
			// psi(0)
